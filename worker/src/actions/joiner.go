...
func (j *Joiner) zetaStage(data []*protocol.Zeta_Data) (tasks Tasks) {
	tasks = make(Tasks)
	tasks[j.infraConfig.Rabbit.MapExchange] = make(map[string]map[string]*protocol.Task)
	tasks[j.infraConfig.Rabbit.MapExchange][ETA_STAGE_1] = make(map[string]*protocol.Task)
	...
	for id, data := range eta1Data {
		tasks[j.infraConfig.Rabbit.MapExchange][ETA_STAGE_1][id] = &protocol.Task{
...
func (j *Joiner) iotaStage(data []*protocol.Iota_Data) (tasks Tasks) {
	tasks = make(Tasks)
	tasks[j.infraConfig.Rabbit.MapExchange] = make(map[string]map[string]*protocol.Task)
	tasks[j.infraConfig.Rabbit.MapExchange][KAPPA_STAGE_1] = make(map[string]*protocol.Task)
	...
	for id, data := range kappa1Data {
		tasks[j.infraConfig.Rabbit.MapExchange][KAPPA_STAGE_1][id] = &protocol.Task{
