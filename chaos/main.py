        ...
        with open(compose_file_path, 'r') as file:
            for line in file:
                ...
                if line.startswith('container_name:'):
                    ...
                    container_name = line.split('container_name:', 1)[1].strip()
...
def loop(func, mu, sigma):
    ...
    try:
        ...
        while True:
            ...
            print(f"{GREEN}Next random node malfunction in {sleep_time:.2f} seconds (iteration {i}){RESET}\n")
            sleep(sleep_time)
            i += 1
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python main.py <mu> <sigma>")
        sys.exit(1)
